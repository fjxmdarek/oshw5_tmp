...
DEFINE_LOCK_GUARD_1(task_rq_lock, struct task_struct,
		    ...
		    task_rq_unlock(_T->rq, _T->lock, &_T->rf), struct rq *rq;
		    struct rq_flags rf)
...
DEFINE_LOCK_GUARD_1(rq_lock, struct rq, rq_lock(_T->lock, &_T->rf),
		    rq_unlock(_T->lock, &_T->rf), struct rq_flags rf)

DEFINE_LOCK_GUARD_1(rq_lock_irq, struct rq, rq_lock_irq(_T->lock, &_T->rf),
		    rq_unlock_irq(_T->lock, &_T->rf), struct rq_flags rf)

DEFINE_LOCK_GUARD_1(rq_lock_irqsave, struct rq,
		    rq_lock_irqsave(_T->lock, &_T->rf),
		    rq_unlock_irqrestore(_T->lock, &_T->rf), struct rq_flags rf)
...
#define DEFINE_LOCK_GUARD_2(name, type, _lock, _unlock, ...)                   \
	...
	static inline class_##name##_t class_##name##_constructor(type *lock,  \
								  type *lock2) \
	{                                                                      \
...
extern int try_to_wake_up(struct task_struct *tsk, unsigned int state,
			  int wake_flags);
...
static inline int mm_cid_get(struct rq *rq, struct mm_struct *mm)
{
	...
	if (mm_cid_is_lazy_put(cid)) {
		if (try_cmpxchg(&this_cpu_ptr(pcpu_cid)->cid, &cid,
				MM_CID_UNSET))
			__mm_cid_put(mm, mm_cid_clear_lazy_put(cid));
	}
...
static inline void sched_mm_cid_migrate_to(struct rq *dst_rq,
